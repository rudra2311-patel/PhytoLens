...
class _MainNavigationScreenState extends State<MainNavigationScreen> {
  ...
  /// Initialize alerts for all existing farms on app start (once per session)
  Future<void> _initializeAlerts() async {
    debugPrint('🚀 Initializing weather alerts for existing farms...');
    try {
      // Check if we already have today's alerts (avoid duplicates)
      final existingAlerts = await FarmDatabaseHelper.instance.getAllAlerts();
      final today = DateTime.now();
      final hasRecentAlert = existingAlerts.any((alert) {
        final alertDate = alert.createdAt;
      ...
      if (hasRecentAlert) {
        debugPrint('ℹ️ Already have alerts for today, skipping initial check');
        return;
      }

      // Create initial alert for weather status (once per day)
      final farms = await FarmDatabaseHelper.instance.getAllFarms();
      if (farms.isNotEmpty) {
        debugPrint('🌾 Found ${farms.length} farm(s), creating alerts...');
        // Create one initial alert per day (not on every refresh)
        await WeatherAlertService.checkAllFarmsWeatherAlerts();
        debugPrint('✅ Initial daily alerts created');
